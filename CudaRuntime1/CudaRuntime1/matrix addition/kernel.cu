...
__global__ void addM(int* Q, int* P, int* N)
{
    int x = blockIdx.x + threadIdx.x;
    int y = blockIdx.y + threadIdx.y;
    printf("my thread id is x: %d and y: %d\n", x, y);
    int index = y * blockDim.x + x; // blockDim has an x and a y and can be used for your rows/colm diminesion
    N[index] = Q[index] + P[index]; // Example operation on the array element

}

int main()
{
    const int rows = 10; //define rows
    const int colloms = 10; //define colloms
    ...
    addM << <blockDim, size >> > (d_A, d_B, d_out); //the kernel launch
